...
정상 주간 보고서 실행은 비공개 Actions Secret `PORTFOLIO_HELD_THEME_IDS`를 추가 입력으로 사용한다. 값은 현재 보유 포트폴리오를 공개 테마 ID로 매핑한 JSON 배열이다. 수량·평가금액·종목명은 포함하지 않는다.

```json
["example_theme_id"]
```

- enabled 테마의 ID만 허용한다.
- 빈 배열, 중복, placeholder, 미등록·disabled ID는 실패한다.
- Secret이 없거나 잘못되면 전체 enabled 테마로 fallback하지 않고 정상 보고서 생성을 중단한다.
- `quality.shouldAbort === true`이면 기존 품질 실패 알림을 유지하기 위해 이 Secret을 요구하지 않는다.
...
7. 정상 주간 보고서는 비공개 입력에 포함된 현재 보유 테마만 평가하고 그중 상위 3개로 제한한다. 독립 합성 모델 실행은 전체 enabled 테마 평가를 지원한다.
